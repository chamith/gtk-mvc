	...
	public class FrontController
	{
		...
		static FrontController()
		{
            ...
            for (int i = 0; i < mvcConfig.Assemblies.Count; i++)
            {
				...
				AssemblyCashItem cashItem = new AssemblyCashItem()
				{
					Name = assConfig.Name,
					ControllerRootNamespace = assConfig.ControllerRootNamespace,
					ViewRootNamespace = assConfig.ViewRootNamespace
				};
		...
		public static void RenderView(string area, string controller, string view, ViewOutputModel output)
		{
			...
			if(viewObj == null)
				throw new ArgumentNullException(
                                string.Format("{0}.{1}.{2}", area, controller, view), 
                                string.Format("View {0}.{1}.{2} cannot be found.", area, controller, view)
                                );
		...
		{
			for(int i=0;i<container.Children.Length;) container.Remove(container.Children[i]);
